...
function populateCountries2(countryElementId2, stateElementId2){
	// given the id of the <select> tag as function argument, it inserts <option> tags

	var countryElement2 = document.getElementById(countryElementId2);
	countryElement2.length=0;
	countryElement2.options[0] = new Option('Select Country','-1');
	countryElement2.selectedIndex = 0;
	for (var i=0; i<country_arr.length; i++) {
		countryElement2.options[countryElement2.length] = new Option(country_arr[i],country_arr[i]);
	}

	// Assigned all countries. Now assign event listener for the states.

	if( stateElementId2 ) {
		/*if(countryElementId == 'countryAdd'){
		 alert('first');
		 }*/
		countryElement2.onclick = function () {
			...
			populateStates2(countryElementId2, stateElementId2);
		};
	}
}
function populateStates2( countryElementId2, stateElementId2 ){

	var selectedCountryIndex2 = document.getElementById( countryElementId2).selectedIndex;

	var stateElement2 = document.getElementById( stateElementId2 );

	stateElement2.length=0;	// Fixed by Julian Woods
	stateElement2.options[0] = new Option('Select State','');
	stateElement2.selectedIndex = 0;

	var state_arr2 = s_a[selectedCountryIndex2].split("|");

	for (var i=0; i<state_arr2.length; i++) {
		stateElement2.options[stateElement2.length] = new Option(state_arr2[i],state_arr2[i]);
	}
}